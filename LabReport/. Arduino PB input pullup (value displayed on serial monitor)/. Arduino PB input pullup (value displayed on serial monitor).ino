/*THIS IS A PUSHBUTTON PULLUP INTERNAL
In this program, when the push button pressed, it must display "0" on the
serial monitor and must turn OFF the LED. Inversely, when the push button is
not pressed, it must display "1" on the serial monitor and must keep the
LED turned ON. Similar to PB PULLUP external, buttonState is 1 when unpressed.*/
...
void setup() {
  ...
  pinMode(button, INPUT_PULLUP);  // This is to assign the button as an INPUT
...
// When the button is pressed serial monitor will display "Button State = value"
  ...
  /* This if/else funtion tuens ON or OFF the LED when push button is unpressed or unpressed*/
  if (buttonState==HIGH){ 		      // If push button is unpressed, buttonState == HIGH
    digitalWrite(LED, HIGH);}       // This turns ON the LED
  else { 					                 	// If push button is pressed, it turns OFF the LED
